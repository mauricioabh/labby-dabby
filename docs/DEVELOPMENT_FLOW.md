...
- **Root Directory (obligatorio):** `apps/web` en Vercel → **Settings → General → Root Directory** → `apps/web` → Save. Sin esto, el deploy busca `.next` en la raíz del repo y falla con `routes-manifest`.
- **Build local:** `npm run build --workspace=web` desde la raíz del monorepo (genera `apps/web/.next`)
- **Install / Build en Vercel:** definidos en `apps/web/vercel.json` (`cd ../.. && npm install` y `npm run build --workspace=web`)
- **Guía paso a paso:** [VERCEL_SETUP.md](./VERCEL_SETUP.md)
